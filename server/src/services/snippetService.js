...
class SnippetService {
  normalizeVisibility(snippetData = {}) {
    const requestedVisibility = String(snippetData.visibility || '').toUpperCase();
    const visibility =
      requestedVisibility && ['PRIVATE', 'TEAM', 'SHARED', 'PUBLIC'].includes(requestedVisibility)
        ? requestedVisibility
        : snippetData.is_public
          ? 'PUBLIC'
          : 'PRIVATE';
    return visibility;
  }
  ...
  async createSnippet(snippetData, userId) {
    try {
      ...
      const visibility = this.normalizeVisibility(snippetData);

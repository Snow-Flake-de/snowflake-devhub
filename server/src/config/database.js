...
import { up_v2_0_0_foundation_security } from "./migrations/20260222-foundation-security.js";
...
function getDatabasePath() {
  if (process.env.BYTESTASH_DB_PATH) {
    const explicitPath = process.env.BYTESTASH_DB_PATH;
  ...
  const dataRoot = process.env.BYTESTASH_DATA_PATH
    ? process.env.BYTESTASH_DATA_PATH
    : join(__dirname, "../../../data/snippets");
  const dbPath = dataRoot;
...
function initializeDatabase() {
  try {
    ...
    if (!dbExists) {
      ...
      up_v2_0_0_foundation_security(db);
